...
class Stacks:
    ...
    """   
    def dictPush(self,d):
        self.dictstack.append(d) # puts the dictionary at the top of stack
    ...
    def define(self,name, value):
        if len(self.dictstack) == 0: # if dictstack is empty, first add empty dictionary to the dictstack
            newdict = dict()
            self.dictstack.append(newdict)
        topdict = self.dictstack[len(self.dictstack) - 1] # get top dictionary in dictstack
        topdict[name] = value # add name:value pair to top dictionary
    ...
    def lookup(self,name):
        if len(self.dictstack) > 0:
            name2 = '/' + name
            for i in range(len(self.dictstack)-1, -1, -1): #iterate from top of dictstack to bottom of dictstack
                if name2 in self.dictstack[i]:
                    return self.dictstack[i].get(name2)

            raise ValueError('Error: lookup - Given variable/function is not found')
        else:
    ...
    def put(self):
        ...
        index = self.opPop()
        x = self.opPop()
        if (isinstance(item, int)) and (isinstance(index, int)) and (isinstance(x, StrConstant)):
            x.value = x.value[:index + 1] + chr(item) + x.value[index + 2:]
        elif (isinstance(item, int)) and (isinstance(index, int)) and (isinstance(x, DictConstant)):
            x.value[index] = item
        else:
            self.opPush(x)
            self.opPush(index)
    ...
    def putinterval(self):
        first_constant = self.opPop()
        start_index = self.opPop() + 1
        second_constant = self.opPop()
        if (start_index + len(second_constant.value)) > (len(first_constant.value)):
            print("Error - end index of the slice goes beyond the string length")
        else:
            first_constant.__delattr__value = first_constant.value[start_index:start_index + len(second_constant.value)]
            self.opPush(first_constant.value)
    ...
    def search(self):
        ...
        #if
        i = 0               
        while i < len(self.opstack): 
            if self.opstack[i] == inputstr: 
                self.opPush(True)
            i = i+1          
    #else:               
